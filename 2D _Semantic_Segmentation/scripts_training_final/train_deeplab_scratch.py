...
def train_one_epoch(loader, model, optimizer, loss_fn):
    model.train()
    loop = tqdm(loader, leave=True, desc="DeepLab Scratch")
    mean_loss = []
...
def main():
    print(f"[INFO] DeepLabV3+ da Scratch (gtFine) — {DEVICE}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_miou > best_miou:
            best_miou = val_miou
            best_path = os.path.join(CHECKPOINT_DIR, "deeplab_scratch_best.pth")
            torch.save(model.state_dict(), best_path)
            print(f"[BEST] mIoU={best_miou:.4f} -> {best_path}")

        # Recovery checkpoint (sovrascrive ogni epoca)
        torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, "deeplab_scratch_latest.pth"))

        if epoch % 10 == 0:
            torch.save(model.state_dict(),
                       os.path.join(CHECKPOINT_DIR, f"deeplab_scratch_epoch_{epoch}.pth"))

    torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, "deeplab_scratch_final.pth"))
